...
from liteai_sdk.tool.toolset import tool, Toolset
...
class TestToolsetDecorator:
    """Test the @tool decorator and Toolset class"""

    # ------------------------------------------------------------------------
    # 7.1 @tool decorator basic functionality
    # ------------------------------------------------------------------------

    def test_tool_decorator_marks_function(self):
        """@tool decorator should mark a function with the tool flag"""
        from liteai_sdk.tool.toolset import TOOL_FLAG
    ...
    def test_tool_decorator_preserves_function(self):
        """@tool decorator should preserve the original function"""
    ...
    def test_tool_decorator_on_method(self):
        """@tool decorator should work on methods"""
        from liteai_sdk.tool.toolset import TOOL_FLAG
...
class TestToolsetBasic:
    ...
    # ------------------------------------------------------------------------
    # 7.2 Toolset with no tools
    # ------------------------------------------------------------------------

    def test_empty_toolset(self):
        """Toolset with no @tool decorated methods should return empty list"""
        ...
        tools = toolset.get_tool_methods()

        assert tools == []

    # ------------------------------------------------------------------------
    # 7.3 Toolset with single tool
    # ------------------------------------------------------------------------

    def test_toolset_single_tool(self):
        """Toolset should discover single @tool decorated method"""
        ...
        assert len(tools) == 1
        assert tools[0].__name__ == "calculate"
        assert callable(tools[0])

    # ------------------------------------------------------------------------
    # 7.4 Toolset with multiple tools
    # ------------------------------------------------------------------------

    def test_toolset_multiple_tools(self):
        """Toolset should discover all @tool decorated methods"""
    ...
    # ------------------------------------------------------------------------
    # 7.5 Tool methods are bound methods
    # ------------------------------------------------------------------------

    def test_toolset_methods_are_bound(self):
        """Tools returned from Toolset should be bound methods"""
        ...
        tools = calc.get_tool_methods()

        assert len(tools) == 1
        # Should be able to call without passing self
        assert tools[0](50) == 150


class TestToolsetIntegration:
    """Test Toolset integration with tool preparation"""

    # ------------------------------------------------------------------------
    # 7.6 prepare_tools with Toolset methods
    # ------------------------------------------------------------------------

    def test_prepare_tools_with_toolset_methods(self):
        """prepare_tools should work with methods from Toolset"""
        ...
        toolset = MathToolset()
        tool_methods = toolset.get_tool_methods()
    ...
    # ------------------------------------------------------------------------
    # 7.7 Mixed tools: functions and Toolset methods
    # ------------------------------------------------------------------------

    def test_prepare_tools_mixed_with_toolset(self):
        """prepare_tools should handle mix of functions and Toolset methods"""
        ...
        toolset = MyToolset()
        all_tools = [standalone_func] + toolset.get_tool_methods()
...
class TestToolsetWithExecution:
    """Test Toolset methods with tool execution"""

    # ------------------------------------------------------------------------
    # 7.8 Execute Toolset method synchronously
    # ------------------------------------------------------------------------

    def test_execute_toolset_method_sync(self):
        """execute_tool_sync should work with Toolset methods"""
        class Calculator(Toolset):
            @tool
    ...
    # ------------------------------------------------------------------------
    # 7.9 Execute Toolset method asynchronously
    # ------------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_execute_toolset_method_async(self):
        """execute_tool should work with Toolset methods"""
        ...
        methods = tools_instance.get_tool_methods()

        result = await execute_tool(methods[0], '{"text": "hello"}')
        assert result == "olleh"

    # ------------------------------------------------------------------------
    # 7.10 Execute async Toolset method
    # ------------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_execute_async_toolset_method(self):
        """Async methods in Toolset should work"""
        ...
        methods = toolset.get_tool_methods()
...
class TestToolsetAdvanced:
    """Advanced Toolset test cases"""

    # ------------------------------------------------------------------------
    # 7.11 Toolset with instance state
    # ------------------------------------------------------------------------

    def test_toolset_with_state(self):
        """Toolset methods should have access to instance state"""
    ...
    # ------------------------------------------------------------------------
    # 7.12 Toolset inheritance
    # ------------------------------------------------------------------------

    def test_toolset_inheritance(self):
        """Child Toolset should inherit parent's tools"""

        class BaseToolset(Toolset):
            @tool
    ...
    # ------------------------------------------------------------------------
    # 7.13 Toolset with complex parameter types
    # ------------------------------------------------------------------------

    def test_toolset_complex_types(self):
        """Toolset methods with complex types should work with prepare_tools"""
        ...
        prepared = prepare_tools(toolset.get_tool_methods())
    ...
    # ------------------------------------------------------------------------
    # 7.14 Multiple Toolset instances
    # ------------------------------------------------------------------------

    def test_multiple_toolset_instances(self):
        """Multiple instances of same Toolset should work independently"""

        class Counter(Toolset):
        ...
        tools1 = counter1.get_tool_methods()
        tools2 = counter2.get_tool_methods()

        # Execute on different instances
        assert tools1[0](5) == 5
        assert tools2[0](5) == 105
...
class TestParamParserWithToolsets:
    ...
    # ------------------------------------------------------------------------
    # 7.15 ParamParser extracts tools from toolsets
    # ------------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------------
    # 7.16 ParamParser with both tools and toolsets
    # ------------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------------
    # 7.17 ParamParser with multiple toolsets
    # ------------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------------
    # 7.18 ParamParser with None toolsets
    # ------------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------------
    # 7.19 ParamParser with empty toolsets list
    # ------------------------------------------------------------------------
...
class TestToolsetEdgeCases:
    """Edge cases and error scenarios for Toolset"""

    # ------------------------------------------------------------------------
    # 7.20 Toolset with private methods
    # ------------------------------------------------------------------------

    def test_toolset_ignores_private_methods(self):
        """Toolset should not include private methods even if decorated"""

        class PrivateToolset(Toolset):
            @tool
    ...
    # ------------------------------------------------------------------------
    # 7.21 Toolset with classmethod/staticmethod
    # ------------------------------------------------------------------------

    def test_toolset_with_classmethod(self):
        """@tool on classmethod should be returned by get_tool_methods"""

        class ToolsetWithClassmethod(Toolset):
            @tool
    ...
    # ------------------------------------------------------------------------
    # 7.22 Toolset method without docstring
    # ------------------------------------------------------------------------

    def test_toolset_method_without_docstring_fails_prepare(self):
        """Toolset method without docstring should fail when preparing"""
        class NoDocToolset(Toolset):
            @tool
