...
def test_parse_thinking_content_with_think_tag_at_start() -> None:
    message = _message("<think>thinking part</think>actual content")

    result = OpenAIProvider._parse_thinking_content(message)

    assert result.content == "actual content"
    assert result.reasoning_content == "thinking part"


def test_parse_thinking_content_with_thinking_tag_at_start() -> None:
...
def test_parse_thinking_content_without_tag() -> None:
...
def test_parse_thinking_content_with_unclosed_tag() -> None:
...
def test_parse_thinking_content_with_tag_not_at_start() -> None:
...
def test_parse_thinking_content_keeps_existing_reasoning_content() -> None:
    message = _message(
